...
if (!isset($_GET['query']) || empty($_GET['query'])) {
    ...
    if ($book === null || count($book) === 0) {
        $templateParams["testo"] = "Spiacenti, la ricerca di '<strong>" . $query . "</strong>' non ha prodotto nessun risultato. Riprova con un altro libro.";
    } else {
        $templateParams["testo"] =  "Risultati per la ricerca di '<strong>" . $query . "</strong>' :"; 
    }
}
